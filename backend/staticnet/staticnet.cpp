...
void SendcoinWorker::process() { 
    ...
    if (xUtility.getSelectedNetworkid() != 0) {
        ...
        if (fGood) {
            ...
            res.insert(0, data);
            QJsonObject response;
            response.insert("sender", QJsonValue::fromVariant("SendcoinWorker::process"));
            response.insert("traceID", QJsonValue::fromVariant(trace_id));
            response.insert("params", QJsonValue::fromVariant(res));
            QMetaObject::invokeMethod(&staticNet, "ResponseFromStaticnet", Qt::DirectConnection, Q_ARG(QJsonObject, response));
            ...
            QJsonObject data;
            data.insert(QStringLiteral("error"), "Bad private key");
            QJsonArray res;
            res.insert(0, data);
            QJsonObject response;
            response.insert("sender", QJsonValue::fromVariant("SendcoinWorker::process"));
        ...
        QJsonObject data;
        data.insert(QStringLiteral("error"), "Bad or mising network ID.");
        QJsonArray res;
        res.insert(0, data);
        QJsonObject response;
        response.insert("sender", QJsonValue::fromVariant("SendcoinWorker::process"));
